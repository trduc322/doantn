...
const FindLaptop = ({ laptops }) => {
  ...
  const handleSort = (e) => {
    let sortData = [...laptopList].sort((a, b) =>
      a[e.target.value] > b[e.target.value] && sortAscending ? 1 : -1
    );
  ...
  return(
          ...
          <div className="col-span-9 my-10 bg-[#e3f5f8] p-5 relative z-0">
            <div className=" flex my-5 items-start ">
              <div className="bg-white w-2/3 rounded-md absolute z-10 shadow-md">
                <div className="flex ">
                  <input
                    className="focus:outline-none w-full px-5 py-2 border-none rounded"
                    placeholder="Search for a laptop ..."
                    onChange={handleChange}
                    value={text}
                  />
                  <img
                    className="pr-2 py-1 self-center w-8 h-8"
                ...
                {suggestion &&
                  suggestion.map((item, index) => (
                    <p
                      ...
                      key={index}
                      onClick={() => {
                        navigate(`/laptopdetails/${item.LaptopId}`);
                      }}
                    >
                      {item.LaptopModel}
                    </p>
              ...
              <div className="w-full text-right">
                <select
                  className="pl-4 pr-10 py-2 ml-5 rounded"
                  onChange={handleSort}
                >
                ...
                <select
                  className="pl-4 pr-10 py-2 ml-3 rounded "
            ...
            <div className="grid grid-cols-4 gap-4 ">
              {laptopList &&
                laptopList.map((item) => (
                  <ItemCard
                    id={item.LaptopId}
                    name={item.LaptopModel}
                    price={item.LaptopPrice}
                    image={item.LaptopImg}
                  />
                ))}
            </div>
          </div>
    ...
    <div className="m-auto">
      <Spinner radius={100} color={"#15b9d5"} stroke={10} visible={true} />
    </div>
  );
};
